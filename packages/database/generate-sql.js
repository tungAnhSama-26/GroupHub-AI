...
let sql = `-- Seed script for Profession Categories and Professions
-- Run this script to populate the database with over 300 professions.

`;

// Generate unique UUIDs
for (const [category, professions] of Object.entries(data)) {
  const catId = crypto.randomUUID();
  sql += `-- ==========================================
-- CATEGORY: ${category}
-- ==========================================
INSERT INTO "ProfessionCategory" ("id", "name", "description", "isActive", "createdAt", "updatedAt")
VALUES ('${catId}', '${category.replace(/'/g, "''")}', NULL, true, NOW(), NOW())
ON CONFLICT ("name") DO NOTHING;

`;

  // We should actually get the id that was just inserted OR existed. 
  // Standard Postgres ON CONFLICT DO NOTHING does not return id if exists without a trick.
  // Instead, since it's a seed script, it's safer to use an DO UPDATE to set id to what we want, or insert it.
  // Actually, to make it robust, we can just do a DO UPDATE SET "name" = EXCLUDED."name" to always get the id.
  // Or better, we can just use subqueries to get the category id by name for the professions!
  
  for (const prof of professions) {
    const profId = crypto.randomUUID();
    sql += `INSERT INTO "Profession" ("id", "name", "description", "isActive", "categoryId", "createdAt", "updatedAt")
VALUES (
  '${profId}', 
  '${prof.replace(/'/g, "''")}', 
  NULL, 
  true, 
  (SELECT "id" FROM "ProfessionCategory" WHERE "name" = '${category.replace(/'/g, "''")}' LIMIT 1), 
  NOW(), 
  NOW()
)
ON CONFLICT ("name") DO NOTHING;
...
fs.writeFileSync('seed-professions.sql', sql);
console.log('Tạo file seed-professions.sql thành công!');
